...
#in CTR files
in_folder = 'C:/Users/cnamgoong/Desktop/Shared/RW/CTR Files/21-JUL-16/'
...
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/22")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/21")) 
                       & ((df_merge['Booking Point'] == 48082) 
                         & (~pd.notnull(df_merge['Cards ID'])))))]
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/22")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/21")) 
                       & ((df_merge['Booking Point'] == 1865) 
                         & (~pd.notnull(df_merge['Cards ID'])))))]
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/25")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/22")) 
                       & ((df_merge['Booking Point'] == 48082) 
                         & (~pd.notnull(df_merge['Cards ID'])))))]
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/25")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/22")) 
                       & ((df_merge['Booking Point'] == 1865) 
                         & (~pd.notnull(df_merge['Cards ID'])))))]
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/26")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/25")) 
                       & ((df_merge['Booking Point'] == 48082) 
                         & (~pd.notnull(df_merge['Cards ID'])))))]
df_merge = df_merge[(~((df_merge['Maturity Date'].str.contains("2016/07/26")) 
                       & (df_merge['Issue Date'].str.contains("2016/07/25")) 
